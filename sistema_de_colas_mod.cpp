...
int main(int argc, char** argv)  /* Funcion Principal */
{
    ...
    if (argc != 4) {
        /* El número esperado de argumentos no coincide */
        fprintf(resultados, "\nNúmero erróneo de argumentos, se requiren 3, llegaron %d", argc);
        exit(2);
    }
    media_entre_llegadas = stof(argv[1], NULL);
    media_atencion = stof(argv[2], NULL);
    num_esperas_requerido = stoi(argv[3], NULL);
...
void reportes(void)  /* Funcion generadora de reportes. */
{
    ...
    fprintf(stdout, "Numero promedio en cola%10.3f\n\n",
            area_num_entra_cola / tiempo_simulacion);
    fprintf(stdout, "Uso del servidor%15.3f\n\n",
            area_estado_servidor / tiempo_simulacion);
    fprintf(stdout, "Tiempo de terminacion de la simulacion%12.3f minutos", tiempo_simulacion);
}
